...
rctbls <- function(hypeR_GEM_enrichments,
  ...
  outer_df = data.frame(signature = names(hypeR_GEM_enrichments),
                        ...
                        enriched = sapply(hypeR_GEM_enrichments, function(x){nrow(x$data)}),
                        gsets = sapply(hypeR_GEM_enrichments, function(x){x$info[["Genesets"]]}),
                        bg = sapply(hypeR_GEM_enrichments, function(x){x$info[["Background"]]}))
