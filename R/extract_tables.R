...
extract_tables <- function(con,
  ...
  if(directional){
    ...
    for(m in 1:ncol(r2m_matrix)){
      reactions <- S %>%
        dplyr::filter(.data$i == m, .data$x > 0) %>% ## i = metabolite, j = reaction, x = flux
        dplyr::pull(.data$j)
    ...
    for(m in 1:ncol(r2m_matrix)){
      reactions <- S %>%
        dplyr::filter(.data$i == m) %>% ## i = metabolite, j = reaction, x = flux
        dplyr::pull(.data$j)
  ...
  m2r <- as.data.frame(r2m_matrix) %>%
    lapply(.data, function(x,y){return(row.names(y)[which(x >0)])}, .data)
